...
# STEP 2: Generating Best Solutions
...
- The solution MUST AVOID over-engineering, exhaustive error handling, checking, testing, or documentation writing
- The solution MUST be simple, intelligent, helpful, and relevant to accomplish the task.

Here is the list of helpful, intelligent, and non complicated observations that will help generate best solutions based on the criteria:
[[number]. ([observation_number]) [Observation]]
...
Here are the best solutions that SIMPLE, DOES NOT ADD UNNECESSARY COMPLEXITY but VERY HELPFUL and RELEVANT to accomplish the task:
[
Numbered list of solutions. Include the observation quote.
Example:
...
## Here is the carefully crafted step-by-step ACTIONABLE coding tutorial to accomplish the task that are SIMPLE, DOES NOT ADD UNNECESSARY COMPLEXITY but VERY HELPFUL and RELEVANT to accomplish the task based on DRY and KISS principle:
[
...
Related solution: <quote>[solution_number]. **[Quote from solution (exclude the observation!)]**</quote>

Related codebase entities:
[Related codebase entities (files/classes/methods/function/properties) in comma]
...
**Here is the summary of added/modified/renamed/deleted files, classes, methods, and functions with changes:**
```txt
[Added/modified/renamed/deleted files, classes, methods, and functions in form of TREE along with reasons for modification]
```
