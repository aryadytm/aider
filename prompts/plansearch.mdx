...
<codeblock ignore_xml_indents_in_codeblocks="{ignore_xml_indents_in_codeblocks}">
[file path]
...
<default_variables note="This can be overrided by using <variables/> tag.">
n_observations = 10
...
.plansearch implement a new feature called Search. The user enters a /search [query] command. And then, return all tasks that contain the query
</user_request>
...
<plansearch ignore_xml_indents_in_response="false">
    ...
    <step4>
        ...
        <results>
            ### Plan Step 1
            Goal: <quote>Implement a new function called `search_notes` that takes a search query as an argument and converts it to lowercase for case-insensitive matching.</quote>
            Key code changes:
            app.py
            ```python
            ...
            ### Plan Step 2
            Goal: <quote>Use the SQL LIKE operator in the search query to allow for partial matching of the search term.</quote>
            Key code changes:
            app.py
            ```python
            def search_notes(query: str):
            ...
            ### Plan Step 3
            Goal: <quote>Modify the SQL query to search for matches in both the title and content columns of the notes table.</quote>
            Key code changes:
            This step is already implemented in the previous step, as we're searching in both title and content columns.

            ### Plan Step 4
            Goal: <quote>Wrap the search term with '%' wildcards in the SQL query to enable partial matching.</quote>
            Key code changes:
            This step is already implemented in step 2, as we've wrapped the search term with '%' wildcards.

            ### Plan Step 5
            Goal: <quote>Format the search results in the same way as the `read_notes` function, displaying the ID, title, and content of each matching note.</quote>
            Key code changes:
            app.py
            ```python
            def search_notes(query: str):
            ...
            ### Plan Step 6
            Goal: <quote>Use parameterized queries when executing the SQL search to prevent SQL injection attacks.</quote>
            Key code changes:
            This step is already implemented in the previous steps, as we're using parameterized queries with `?` placeholders.

            ### Plan Step 7
            Goal: <quote>Add error handling for empty search queries and display an appropriate message when no results are found.</quote>
            Key code changes:
            app.py
            ```python
            def search_notes(query: str):
            ...
            ### Plan Step 8
            Goal: <quote>Implement the search function to be both case-insensitive and use partial matching for maximum flexibility.</quote>
            Key code changes:
            ...
            ### Plan Step 9
            Goal: <quote>Add a new elif condition in the run_cli function to handle the "/search" command, which will call the new search_notes function.</quote>
            Key code changes:
            app.py
            ```python
            def run_cli():
            ...
            ### Plan Step 10
            Goal: <quote>Update the help message to include information about the new search feature.</quote>
            Key code changes:
            app.py
            ```python
            def run_cli():
